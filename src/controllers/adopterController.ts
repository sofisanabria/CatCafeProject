...
const AdopterSchema = z.object({
  ...
  phone: z.string().trim().regex(/^\d+$/, "Phone number must contain only digits")
  .refine((num) => num.length >= 8, {message: "Phone number must have at least 8 digits"})
  .transform((val) => Number(val.replace(/^0+/, ""))), // Remove leading zeros and convert to number
...
export class AdopterController {
  ...
  async removeAdopter(req: Request, res: Response) {
    ...
    try {
      ...
      if (adoptedCats.length >= 1) { return res.status(400).json({message: 'Adopters can not be deleted if the adoption was carried out'});}
